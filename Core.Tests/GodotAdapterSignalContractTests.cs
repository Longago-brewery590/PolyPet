...
namespace PolyPet.Tests
{
    public class GodotAdapterSignalContractTests
    {
        ...
        public void PolyPetAvatar_DeclaresGodotSignalsInsteadOfPlainEvents()
        {
            ...
            Assert.Contains("public delegate void SeedChangedEventHandler(PolyPetAvatar avatar, Variant seed);", source);
            Assert.Contains("public delegate void NameSeedChangedEventHandler(PolyPetAvatar avatar, Variant nameSeed);", source);
        ...
        public void UnityPolyPetAvatar_UsesSerializedTypedUnityEventsInsteadOfPlainEvents()
        {
            ...
            Assert.Contains("public sealed class AvatarNullableIntEvent : UnityEvent<PolyPetAvatar, NullableInt>", source);
            Assert.Contains("public delegate void SeedChangedCallback(PolyPetAvatar avatar, NullableInt seed);", source);
            Assert.Contains("public delegate void NameSeedChangedCallback(PolyPetAvatar avatar, NullableInt nameSeed);", source);
            Assert.Contains("[SerializeField] private AvatarNullableIntEvent _seedChanged = new AvatarNullableIntEvent();", source);
            Assert.Contains("[SerializeField] private AvatarNullableIntEvent _nameSeedChanged = new AvatarNullableIntEvent();", source);
